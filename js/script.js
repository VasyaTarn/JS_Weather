...
{
    const apiKey = '540fab740a87c5653742fc9093173ffd';

    return fetch(`https://api.openweathermap.org/data/2.5/weather?lat=${latitude}&lon=${longitude}&units=metric&appid=${apiKey}`)
...
function getWeatherByLocation() 
{
    getUserLocation()
        ...
        .then(({ location, cityName }) => {
            ...
            fetch(`https://api.openweathermap.org/data/2.5/weather?lat=${location.latitude}&lon=${location.longitude}&units=metric&appid=${apiKey}`)
                .then(response => response.json())
                .then(data => {
                    updateCurrentWeather(data);

                    return fetch(`https://api.openweathermap.org/data/2.5/forecast?lat=${location.latitude}&lon=${location.longitude}&units=metric&appid=${apiKey}`);
                })
                .then(response => response.json())
                .then(hourlyData => {
...
function get5DayByCity(city) 
{
    ...
    return fetch(`https://api.openweathermap.org/data/2.5/weather?q=${city}&units=metric&appid=${apiKey}`)
        ...
        .then(locationData => {
            return fetch(`https://api.openweathermap.org/data/2.5/forecast?lat=${locationData.coord.lat}&lon=${locationData.coord.lon}&units=metric&appid=${apiKey}`);
        })
        .then(response => response.json())
        .then(data => {
            update5DayForecast(data);
